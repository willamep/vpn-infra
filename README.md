...
```
vpn-infra/
├── generate_inventory.py         # Inventory generator for VMs created without Terraform
...
└── ansible/                   # Node configuration
    ...
    ├── inventory/hosts.yml    # Generated by Terraform or generate_inventory.py
...
If the nodes were provisioned without Terraform, generate the same inventory from the project root:

```bash
./generate_inventory.py
```

The script interactively asks for the address and SSH user of `vpn-entry` and `vpn-exit`. It also supports non-interactive use:
...
Run `./generate_inventory.py --help` to see all options. The generated file is written to `ansible/inventory/hosts.yml`.
...
- SSH access to both nodes using a key or password.
- SSH access as `root`, or as a user with `sudo` privileges. If `sudo` requires a password, Ansible can prompt for it or read it from an encrypted Ansible Vault variable.
...
- A public SSH key for the `ansible` user.

### 1. Terraform — provision the VMs

> Skip this step if `generate_inventory.py` was used.
...
# Prepare the secrets:
#   - create the vault files and add secret variables to them
...
The commands above assume SSH key authentication and either direct `root` access or passwordless `sudo`. For SSH password authentication, add `--ask-pass` (`-k`). If `sudo` also requires a password, add `--ask-become-pass` (`-K`):
...
The prompts accept one SSH password and one privilege-escalation password for the whole run. If the nodes use different passwords, or for unattended authentication, define `ansible_password` and, when needed, `ansible_become_password` in the encrypted per-host Vault files. Do not store these passwords in plaintext in the inventory or `vars.yml`.
...
> Be careful with the firewall rules: they may disrupt other services running on your nodes.
